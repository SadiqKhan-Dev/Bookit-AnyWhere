...
export const metadata: Metadata = {
  title: "Photography Services — Book a Photographer",
  description: "Discover and book professional photographers. Wedding, portrait, event & commercial photography.",
};
...
async function PhotographyListings({ searchParams }: PhotographyPageProps) {
  ...
  return (
    <>
      <p className="text-sm text-gray-500 mb-6">{total} photographers found</p>
...
export default function PhotographyPage({ searchParams }: PhotographyPageProps) {
    ...
    <div className="min-h-screen">
      ...
      <div className="bg-gradient-to-br from-purple-50 to-pink-50 border-b">
        <div className="container mx-auto px-4 py-10">
          ...
          <p className="text-gray-500">Wedding, portrait, event & professional photography</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-64 shrink-0">
            <ListingsFilter type="PHOTOGRAPHY" searchParams={searchParams} />
          </aside>
          <main className="flex-1">
            ...
            {isMap ? (
              <ListingsMapView type="PHOTOGRAPHY" searchParams={searchParams} />
            ) : (
              <Suspense fallback={<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({ length: 9 }).map((_, i) => <ListingCardSkeleton key={i} />)}</div>}>
                <PhotographyListings searchParams={searchParams} />
              </Suspense>
            )}
          </main>
        </div>
