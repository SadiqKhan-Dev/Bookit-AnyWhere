...
export const metadata: Metadata = {
  title: "Auto Mechanic Services — Book Car Repair",
  description: "Discover and book trusted auto mechanics near you. Car repair, oil change, brake service & auto maintenance.",
};
...
async function MechanicListings({ searchParams }: MechanicsPageProps) {
  ...
  if (!listings.length) {
    return (
      <div className="text-center py-20">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">No mechanics found</h3>
  ...
  return (
    <>
      <p className="text-sm text-gray-500 mb-6">{total} mechanics found</p>
...
export default function MechanicsPage({ searchParams }: MechanicsPageProps) {
  const isMap = searchParams.view === "map";
  return (
    <div className="min-h-screen">
      <Navbar />
      <div className="bg-gradient-to-br from-slate-50 to-gray-50 border-b">
        <div className="container mx-auto px-4 py-10">
          <div className="flex items-center gap-3 mb-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-500">
              <Wrench className="h-5 w-5 text-white" />
            </div>
            <h1 className="text-3xl font-bold text-gray-900">Auto Mechanic Services</h1>
          </div>
          <p className="text-gray-500">Car repair, oil change, brake service & auto maintenance</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-64 shrink-0">
            <ListingsFilter type="MECHANIC" searchParams={searchParams} />
          </aside>
          <main className="flex-1">
            ...
            {isMap ? (
              <ListingsMapView type="MECHANIC" searchParams={searchParams} />
            ) : (
              <Suspense fallback={<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({ length: 9 }).map((_, i) => <ListingCardSkeleton key={i} />)}</div>}>
                <MechanicListings searchParams={searchParams} />
              </Suspense>
            )}
          </main>
        </div>
