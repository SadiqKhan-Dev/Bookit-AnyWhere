...
export const metadata: Metadata = {
  title: "Musicians & Entertainment — Book Live Music",
  description: "Live music, DJs, bands & entertainment for events",
};
...
async function MusicianListings({ searchParams }: MusiciansPageProps) {
  ...
  if (!listings.length) {
    return (
      <div className="text-center py-20">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">No musicians found</h3>
  ...
  return (
    <>
      <p className="text-sm text-gray-500 mb-6">{total} musicians found</p>
...
export default function MusiciansPage({ searchParams }: MusiciansPageProps) {
      ...
      <div className="bg-gradient-to-br from-indigo-50 to-purple-50 border-b">
        <div className="container mx-auto px-4 py-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Musicians & Entertainment</h1>
          </div>
          <p className="text-gray-500">Live music, DJs, bands & entertainment for events</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-64 shrink-0">
            <ListingsFilter type="MUSICIAN" searchParams={searchParams} />
          </aside>
          <main className="flex-1">
            ...
            {isMap ? (
              <ListingsMapView type="MUSICIAN" searchParams={searchParams} />
            ) : (
              <Suspense fallback={<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({ length: 9 }).map((_, i) => <ListingCardSkeleton key={i} />)}</div>}>
                <MusicianListings searchParams={searchParams} />
              </Suspense>
            )}
          </main>
        </div>
