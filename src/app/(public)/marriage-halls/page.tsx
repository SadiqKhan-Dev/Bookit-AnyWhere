...
export const metadata: Metadata = {
  title: "Wedding Venues & Marriage Halls — Book Your Dream Venue",
  description: "Discover and book grand halls, banquet centers & wedding venues for your special day.",
};

interface MarriageHallsPageProps {
  searchParams: {
...
async function MarriageHallsListings({ searchParams }: MarriageHallsPageProps) {
  ...
  if (!listings.length) {
    return (
      <div className="text-center py-20">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">No wedding venues found</h3>
  ...
  return (
    <>
      <p className="text-sm text-gray-500 mb-6">{total} venues found</p>
...
export default function MarriageHallsPage({ searchParams }: MarriageHallsPageProps) {
      ...
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 border-b">
        <div className="container mx-auto px-4 py-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h1 className="text-3xl font-bold text-gray-900">Wedding Venues & Marriage Halls</h1>
          </div>
          <p className="text-gray-500">Grand halls, banquet centers & wedding venues — book your dream venue</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-64 shrink-0">
            <ListingsFilter type="MARRIAGE_HALL" searchParams={searchParams} />
          </aside>
          <main className="flex-1">
            ...
            {isMap ? (
              <ListingsMapView type="MARRIAGE_HALL" searchParams={searchParams} />
            ) : (
              <Suspense fallback={<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({ length: 9 }).map((_, i) => <ListingCardSkeleton key={i} />)}</div>}>
                <MarriageHallsListings searchParams={searchParams} />
              </Suspense>
            )}
          </main>
        </div>
