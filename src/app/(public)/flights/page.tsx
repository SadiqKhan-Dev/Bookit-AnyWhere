...
export const metadata: Metadata = {
  title: "Flights — Book Domestic & International",
  description: "Search and book flights across hundreds of routes with Economy, Business, and First Class seats.",
};

interface FlightsPageProps {
  searchParams: {
    q?: string; city?: string; minPrice?: string; maxPrice?: string;
    rating?: string; tag?: string; page?: string; sort?: string; view?: string;
  };
}

async function FlightListings({ searchParams }: FlightsPageProps) {
  ...
  return (
    <>
      <p className="text-sm text-gray-500 mb-6">{total} flights found</p>
...
export default function FlightsPage({ searchParams }: FlightsPageProps) {
  ...
  return (
    <div className="min-h-screen">
      <Navbar />
      <div className="bg-gradient-to-br from-violet-50 to-purple-50 border-b">
        <div className="container mx-auto px-4 py-10">
          ...
          <p className="text-gray-500">Economy, Business & First Class — domestic and international routes</p>
        </div>
      </div>
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-64 shrink-0">
            <ListingsFilter type="FLIGHT" searchParams={searchParams} />
          </aside>
          <main className="flex-1">
            ...
            {isMap ? (
              <ListingsMapView type="FLIGHT" searchParams={searchParams} />
            ) : (
              <Suspense fallback={<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">{Array.from({ length: 9 }).map((_, i) => <ListingCardSkeleton key={i} />)}</div>}>
                <FlightListings searchParams={searchParams} />
              </Suspense>
            )}
          </main>
        </div>
